...
class TorrentScraperServiceTest extends \PHPUnit_Framework_TestCase
{
    public function testIsSettingAndGettingTheAdapter()
    {
        ...
        $this->assertInstanceOf('\SergeySMoiseev\TorrentScraper\Adapter\NullAdapter', $actual[0]);
    }

    public function testIsSearchingInTheAdapters()
    {
        $expected = [new SearchResult()];
        $adapter1 = $this->getMock('SergeySMoiseev\TorrentScraper\AdapterInterface');
        $adapter1->expects($this->once())
            ->method('search')
            ->with('The Walking Dead S05E08')
            ->willReturn($expected);

        $adapter2 = $this->getMock('SergeySMoiseev\TorrentScraper\AdapterInterface');
    ...
    {
        $adapterMock = $this->getMock('SergeySMoiseev\TorrentScraper\AdapterInterface');
