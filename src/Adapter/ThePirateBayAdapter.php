...
//        }
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, "https://app.scrapinghub.com/api/jobs/list.json?project=$project&spider=tpb&state=finished&count=1");
      ...
      foreach ($items as $item) {
          foreach ($item->torrents as $torrent) {
              $result = new SearchResult();
              $result->setCategory($item->category . ':' . $torrent->sub_category);
              $result->setName($torrent->title);
              $result->setDetailsUrl($torrent->url);
              $result->setSeeders((int)$torrent->seeds);
              $result->setLeechers($torrent->peers);
              $result->setTimestamp((int)$torrent->released);
              $result->setSource(self::ADAPTER_NAME);
              $result->setMagnetUrl($torrent->magnet);
              $result->setSize($torrent->size);
              $result->setIsVerified($torrent->verified);
              $results[] = $result;
          }
      }
